        ...
        SendPost(playbackUrl, deviceProfile, new JJCallBack() {
            ...
            public void onSuccess(String str) {
                try {
                    ...
                    final JsonArray mediaSources = jsonObject.getAsJsonArray("MediaSources");
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
        }, null);
        String playPath = ""; // Declare the playPath variable

        // 检查MediaSources是否为空
        if (mediaSources != null && mediaSources.size() > 0) {
            JsonObject mediaSource = mediaSources.get(0).getAsJsonObject();
            playPath = mediaSource.get("TranscodingUrl").getAsString();
        }
